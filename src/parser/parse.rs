...
pub fn load_xml_recursive(data: &[u8], idx: usize) -> (usize, Vec<Tag>) {
        ...
        if data[i] == '<' as u8 {
            ...
            if !text_buffer.trim().is_empty() && tag.name != "" {
                if !tag.text_content.is_empty() {
                    ...
                    tag.text_content = text_buffer.clone();
                }
            }
            text_buffer.clear();
            ...
            if data[i - 1] == '/' as u8 {
                ...
                if !tag.name.is_empty() {
                    let (next_i, children) = load_xml_recursive(data, i + 1);
            ...
            name.push(data[i]);
        } else if !tag.name.is_empty() && !name_start && !params_start {
